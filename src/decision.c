...
CreateProposalList *decision_evaluate_creates(
    ...
    /* Step 1: baselines (no hypothetical indexes installed). */
    ...
    for (int i = 0; i < cands->n; i++) {
        ...
        for (int q = 0; q < queries->n; q++) {
            ...
            if (delta <= 0) continue; /* planner didn't pick the hypo idx */
        ...
        if (p->benefit <= 0) {
            p->accepted = 0;
            snprintf(p->reason, sizeof(p->reason),
                     "no planner benefit detected");
        } else if (ratio < cfg->min_benefit_ratio) {
            p->accepted = 0;
            snprintf(p->reason, sizeof(p->reason),
                     "benefit/overhead = %.2f < threshold %.2f",
                     ratio, cfg->min_benefit_ratio);
        } else {
            p->accepted = 1;
            snprintf(p->reason, sizeof(p->reason),
                     "benefit/overhead = %.2f >= %.2f",
                     ratio, cfg->min_benefit_ratio);
        }
        out->n++;
    }
...
DropProposalList *decision_evaluate_drops(
    ...
    /* Reasonable per-scan benefit when we can't measure it directly. The
       planner's typical "winning index path" costs roughly tens-to-hundreds
       of cost units for an OLTP-shaped query. We use 100. Tune via config
       if desired (left as a constant here for clarity).                   */
