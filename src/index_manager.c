...
/* "CREATE INDEX ON x.y (a,b)"  ->  "CREATE INDEX CONCURRENTLY auto_xxx ON x.y (a,b)"
   We don't try to be clever with naming; we delegate uniqueness to a small
   hash of the input plus a random suffix.                                  */
...
int index_create_concurrent(DB *db, const char *create_index_sql) {
    ...
    char name[64];
    snprintf(name, sizeof(name), "pgai_%lx",
             simple_hash(create_index_sql) & 0xFFFFFFFFul);

    /* Splice in CONCURRENTLY <name> after CREATE INDEX. */
    const char *rest = create_index_sql + 12;       /* skip "CREATE INDEX" */
